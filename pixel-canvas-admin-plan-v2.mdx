# Pixel Canvas — Admin Controls + Screenshot Implementation Plan
...
No additional env vars needed for admin. Admin URL: `?admin=1`

---

## Phase 1 — Lock State Infrastructure

### 1a. `src/lib/constants.ts` — Add event constants
...
### 1b. `src/hooks/usePixelGrid.ts` — Core lock state changes
...
**On init — update the existing fetch:**
...
**New broadcast handler — add in the channel subscription chain:**
```ts
...
### 1c. `src/App.tsx` — Admin detection + pass `isAdmin` to hook
...
### 2b. `src/App.tsx` — Paused banner for non-admin users
...
### 2c. `src/components/ColorPicker.tsx` — Conditional clear button
...
### 2d. `src/components/PixelCanvas.tsx` — Read-only mode
...
In `handlePointerDown`, `handlePointerMove` — add early return:
...
### 3c. `src/App.tsx` — Screenshot button (all users)
...
This works regardless of lock state — users can screenshot while paused or open.

**Retina note:** The canvas is already scaled for `devicePixelRatio` (see `resizeCanvas` in PixelCanvas.tsx). The exported PNG will be at retina resolution — this is a feature, not a bug (higher quality export).
...
| # | Test | Expected |
|---|------|----------|
| 1 | Regular URL — no admin controls visible | ✅ Only color picker (no clear button) + screenshot button |
| 2 | Admin URL — admin toolbar visible | ✅ Pause/Start + Clear buttons, plus clear button in color picker |
| 3 | Admin pauses → regular user cannot paint | ✅ Clicks do nothing, cursor shows not-allowed, paused banner visible |
| 4 | Admin resumes → regular user regains paint | ✅ Immediate, no refresh needed |
| 5 | Admin clears while paused | ✅ Canvas clears for everyone, persists to DB |
| 6 | New tab joins while paused | ✅ Loads existing grid (read-only) + sees paused banner |
| 7 | Refresh preserves lock state | ✅ Lock state loaded from DB on init |
| 8 | Screenshot while open (regular user) | ✅ PNG downloads, contains only canvas pixels |
| 9 | Screenshot while paused (regular user) | ✅ PNG downloads, no pause overlay in image |
| 10 | In-flight paint batch discarded on pause | ✅ No stale cells arrive after admin pauses |
| 11 | Participant count still works | ✅ Presence tracking unaffected |
| 12 | Admin can paint while paused | ✅ Admin bypasses lock |
...
| `src/lib/exportCanvas.ts` | **New** — screenshot utility |
...
| `.env.local` | No changes for admin |
...
- ✅/❌ for each sub-task listed above
